...
export default function QuadrasPublicas() {
  ...
  const { data: quadrasList = [] } = useQuery<any[]>({
    queryKey: ["/api/quadras/public", arenaId],
    queryFn: () => fetch(`/api/quadras/publico/${arenaId}`).then((r) => r.json()),
  });
      ...
      <div className="bg-white border-b shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Disponibilidade de Quadras</p>
          </div>
        </div>
      </div>
        ...
        {quadrasAtivas.length === 0 ? (
          ...
          <div className="space-y-6">
            {quadrasAtivas.map((quadra: any) => (
              <div key={quadra.id} className="bg-white rounded-xl border shadow-sm overflow-hidden">
                {/* Court header */}
                <div className="flex items-center gap-3 px-4 py-3 border-b" style={{ borderLeftColor: quadra.cor, borderLeftWidth: 4 }}>
                  <div className="h-3 w-3 rounded-full" style={{ backgroundColor: quadra.cor }} />
                  <div>
                    <h3 className="font-semibold text-gray-900">{quadra.nome}</h3>
                    {quadra.descricao && <p className="text-xs text-gray-500">{quadra.descricao}</p>}
                  </div>
                </div>
                  ...
                  <div className="grid min-w-[700px]" style={{ gridTemplateColumns: `repeat(7, 1fr)` }}>
                    {weekDays.map((day, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`border-r last:border-r-0 min-h-[120px] ${isPast ? "bg-gray-50" : ""}`}
                        >
                          ...
                          <div className="p-1.5 space-y-1">
                            {reservas.length === 0 ? (
                              ...
                              reservas.map((r: any) => (
                                <div
                                  key={r.id}
                                  className={`rounded px-1.5 py-1 text-xs border ${TIPO_COLORS[r.tipo] ?? "bg-gray-100 text-gray-700 border-gray-200"}`}
                                >
