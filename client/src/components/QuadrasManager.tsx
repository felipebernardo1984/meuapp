...
export default function QuadrasManager({ arenaId }: QuadrasManagerProps) {
  ...
  const { data: ambientesList = [] } = useQuery<Ambiente[]>({ queryKey: ["/api/recursos"] });

  const criarAmbiente = useMutation({
    mutationFn: (nome: string) => apiRequest("POST", "/api/recursos", { nome, ativo: true }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/recursos"] }); setNovoNome(""); toast({ title: "Ambiente criado!" }); },
    onError: () => toast({ title: "Erro ao criar ambiente", variant: "destructive" }),
  });

  const atualizarAmbiente = useMutation({
    mutationFn: ({ id, data }: { id: string; data: typeof emptyEdit & { ativo: boolean } }) =>
  ...
  const removerAmbiente = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["/api/recursos"] }); setConfirmDelete(null); toast({ title: "Ambiente removido." }); },
  ...
  const handleSalvar = (a: Ambiente) => {
    ...
    atualizarAmbiente.mutate({
      id: a.id,
      data: { nome, valorAluguel: editForm.valorAluguel, valorDayuse: editForm.valorDayuse, ativo: true },
    });
  };
    ...
    <div className="p-4 md:p-6 flex flex-col gap-5 overflow-y-auto flex-1 min-h-0">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Quadras, boxes e salas disponíveis para aulas e reservas. Configure também os valores padrão de aluguel e day-use por ambiente.
        </p>
      </div>
      ...
      <div className="flex gap-2">
        ...
        />
        <Button
          onClick={() => { const nome = novoNome.trim(); if (nome) criarAmbiente.mutate(nome); }}
                ...
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  {isEditing ? (
                    <input
                      ...
                      autoFocus
                      onKeyDown={(e) => {
                        if (e.key === "Escape") { setEditandoId(null); setEditForm(emptyEdit); }
                      }}
                    />
                  ) : (
                  ...
                  {isEditing ? (
                    <>
                      <Button size="sm" onClick={() => handleSalvar(a)} disabled={atualizarAmbiente.isPending} data-testid={`button-salvar-ambiente-${a.id}`}>
                        Salvar
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => { setEditandoId(null); setEditForm(emptyEdit); }}>
                        Cancelar
                      </Button>
                    </>
                  ) : (
                  ...
                  <div className="border-t border-border px-4 py-3 grid grid-cols-2 gap-3 bg-muted/30">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditForm((p) => ({ ...p, valorAluguel: e.target.value }))}
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditForm((p) => ({ ...p, valorDayuse: e.target.value }))}
                  ...
                  <div className="border-t border-border px-4 py-2 flex gap-4 bg-muted/20">
                    {a.valorAluguel && (
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                        ...
                        Aluguel: <span className="font-medium text-foreground">{a.valorAluguel}</span>
                      </span>
                    )}
                    {a.valorDayuse && (
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                        <DollarSign className="h-3 w-3" />
                        Day-use: <span className="font-medium text-foreground">{a.valorDayuse}</span>
                      </span>
                    )}
                  </div>
                ) : null}
        ...
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive"
              onClick={() => { if (confirmDelete) removerAmbiente.mutate(confirmDelete.id); }}
